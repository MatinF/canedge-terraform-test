...
# For Azure, we use the same storage client for both storage and notification operations
# The client is passed to the cloud_functions module where queue operations are performed
storage_client = BlobServiceClient.from_connection_string(storage_connection_string)

# For queue-based notifications, we pass the storage client as the notification client
# This way we can use the same client for both blob and queue operations
notification_client = storage_client
