...
def publish_notification(cloud, client, subject, message, logger):
    ...
    Args:
        cloud (str): Cloud provider ("Amazon", "Google", or "Azure")
        client: Cloud notification client (or storage client for Azure)
    ...
    if cloud == "Amazon":
        ...
        try:
            ...
            logger.warning(f"Published event message to SNS")
            return True
        except Exception as e:
        ...
        try:
            queue_name = os.environ.get("EventQueueName", None)
            if queue_name is None:
                logger.warning("EventQueueName not configured, cannot send to queue")
                return False
            ...
            logger.info(f"Published message to Azure Storage Queue: {queue_name}")
            return True
        except Exception as e:
            logger.error(f"Error publishing to Azure Storage Queue: {e}")
            return False
    else:
        logger.error(f"Unsupported cloud provider: {cloud}")
        return False
